...
for i=1:size(Daily_Stats,1)
    ...
    if current_country_number ~= previous_country_number && size(Training_Input,2) >= 15 && previous_country_number ~= 163 
        ...
        net = feedforwardnet([hidden_neurons1,hidden_neurons2]);
        ...
        % view(net)
        net.IW{1} = rand(hidden_neurons1, 2);
        net.b{1} =  rand(hidden_neurons1, 1);
        net.LW{2,1} =  rand(hidden_neurons2, hidden_neurons1);
        net.b{2} =  rand(hidden_neurons2, 1);
        net.LW{3,2} =  rand(1, hidden_neurons2);
        net.b{3} =  rand(1, 1);
        ...
        filename = strcat('E:\AGH\SSN\Projekt\SSN_Projekt\Cases_4\',country_name,'_training.txt');
        % Write data to text file
        writetable(Training,filename,'Delimiter',' ');
        
        
        Day  = Test_Days';
        Cases = Test_Prediction_Collective(2,:)';
        Predictions = table(Day, Cases);
        country_name = strrep(country_name,'\',' ');
        filename = strcat('E:\AGH\SSN\Projekt\SSN_Projekt\Cases_4\',country_name,'_prediction.txt');
        % Write data to text file
    ...
    if Daily_Stats(i,3) >= 13
        if mod(number_of_valid_data,3) == 0 
            Training_Input(1, j) = Daily_Stats(i,1);
            Training_Input(2,j) = Daily_Stats(i,2);
            Training_Input(3,j) = Daily_Stats(i-3, 3);
            Training_Output(1,j) = Daily_Stats(i,1);
            Training_Output(2,j) = Daily_Stats(i,3);
            j = j+1;
        end
...
filename = strcat('E:\AGH\SSN\Projekt\SSN_Projekt\Cases_4\Performance.txt');
% Write data to text file
